...
function renderCurrentUserOptions() {
  ...
  const optionMarkup = activeMembers
    .map((member) => {
...
function updateCurrentUserUI() {
  ...
  if (topbarProfileInitial) {
    topbarProfileInitial.textContent =
      member.initials || member.displayName.slice(0, 1).toUpperCase();
  }
...
function findStandupForUserDate(userId, standupDate) {
  return appState.standups.find(
    (standup) => standup.userId === userId && standup.standupDate === standupDate
  );
}
...
async function loadFeed(preferredDate) {
  ...
  try {
    ...
    const dates = [
      ...new Set(appState.standups.map((standup) => standup.standupDate).filter(Boolean)),
    ].sort((left, right) => {
    ...
    if (!filteredStandups.length) {
      feedList.innerHTML =
        '<p class="feed-loading">No standups have been submitted for this day yet.</p>';
    }
    ...
    feedList.innerHTML =
      '<p class="feed-error" role="alert">Could not load standup entries. Please refresh.</p>';
    feedList.setAttribute('aria-busy', 'false');
  ...
  return Object.fromEntries(
    rows
      .filter(
        (row) =>
          row.userId === userId &&
          row.dayIndex < MEETING_DAYS.length &&
          row.slotIndex < MEETING_SLOTS.length
      )
...
function getOverlapMap() {
  return new Map(
    appState.overlapSlots.map((slot) => [slotKey(slot.dayIndex, slot.slotIndex), slot])
  );
}
...
async function loadAvailabilityData() {
  ...
  myMeetingAvailability = buildAvailabilityLookup(
    appState.availabilityRows,
...
function renderMeetingOverlap() {
  ...
  if (!appState.overlapSlots.length) {
    meetingOverlapList.innerHTML =
      '<li class="meeting-overlap-item">No saved availability yet for this week.</li>';
  }
}
...
function renderRepoPulse() {
  ...
  const metrics = [
    {
      label: 'Open issues',
      value: repoPulse?.openIssues ?? 0,
      tone: 'neutral',
    },
    {
      label: 'Blocked updates',
      value: repoPulse?.blockedUpdates ?? 0,
      tone: 'warning',
    },
    {
      label: 'Failing workflows',
      value: repoPulse?.failingWorkflows ?? 0,
      tone: (repoPulse?.failingWorkflows ?? 0) ? 'danger' : 'success',
    },
    {
      label: 'Due in 48h',
      value: repoPulse?.dueSoon ?? 0,
      tone: (repoPulse?.dueSoon ?? 0) ? 'warning' : 'success',
    },
  ];
...
function renderAISummary() {
  ...
  if (summaryBlockers) {
    ...
    summaryBlockers.innerHTML = blockers
      .map(
        (item) => `<article class="priority-item priority-item--warning"><p>${item}</p></article>`
      )
      .join('');
  }
  ...
  if (summaryActions) {
    ...
    summaryActions.innerHTML = actions
      .map((item) => `<article class="priority-item"><p>${item}</p></article>`)
      .join('');
  }

  if (meetingBrief) {
    const brief = summary?.brief || [];
    meetingBrief.innerHTML = brief
      .map((item) => {
...
function renderSprintHealth() {
  ...
  if (deadlineRiskList) {
    ...
    deadlineRiskList.innerHTML = riskIssues
      .map(
        (issue) => `
    ...
    if (!riskIssues.length) {
      deadlineRiskList.innerHTML =
        '<p class="feed-loading">No deadline risks are currently flagged.</p>';
    }
  }

  if (workflowTrend) {
    const workflows = sprintHealth?.workflowTrend || [];
    workflowTrend.innerHTML = workflows
      .map(
        (workflow) => `
    ...
    if (!workflows.length) {
      workflowTrend.innerHTML =
        '<p class="feed-loading">No workflow trend data is available yet.</p>';
    }
  }
  ...
  if (issueDistribution) {
    ...
    issueDistribution.innerHTML = distribution
      .map(
        (item) => `
    ...
    if (!distribution.length) {
      issueDistribution.innerHTML =
        '<p class="feed-loading">No issue distribution data is available yet.</p>';
    }
  }
}
